<template>
  <div v-if="isLoggedIn" class="flex flex-col h-dvh">
    ...
    <div class="flex justify-center items-center h-full">  
      <div class="w-4/5 h-3/4 bg-white rounded border border-gray-00 flex flex-col py-3 px-3">
        <div class="flex flex-row justify-between items-center">
          <h1 class="text-2xl font-bold mb-5 mt-3 ml-3">
            Companys List
          </h1>
          <button @click="showNew = true" class="bg-colorAccent hover:bg-blue-800 rounded w-20 duration-300 text-white flex justify-center p-1">
  ...
  <path d="M12 8V16M8 12H16M7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V7.8C21 6.11984 21 5.27976 20.673 4.63803C20.3854 4.07354 19.9265 3.6146 19.362 3.32698C18.7202 3 17.8802 3 16.2 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <table class="w-full text-md bg-gray-100 marker: rounded mb-4">
          <tbody>
              ...
              <tr  v-for="userData in userData" :key="userData.data_id" class="border-b border-gray-300 hover:bg-blue-100 duration-300 bg-white">
                  <td class="p-3 px-5 flex flex-row">
                    <img :src="userData.avatarUrl" class="w-10 h-10 rounded-full mr-4" alt="Avatar">
                    <button @click="showInfo = true" class="font-thin text-lg mt-1 hover:underline">{{ userData.compname }}</button>
                  </td>
                  ...
                  <td class="p-3 px-5 border-l border-gray-300">
                    <button v-on:click="test"  class=" duration-300 text-white p-2 rounded-full inline-flex items-center mr-2 hover:bg-green-300">
                    ...
                    <button class="duration-300 text-white inline-flex items-center p-2 rounded-full hover:bg-blue-300">
                    ...
                    <button @click="handleDelete(userData.data_id)" class="duration-300 text-white rounded-full ml-2 p-2 inline-flex items-center hover:bg-red-300">
          ...
          <newModal :isVisible="showNew" @close="showNew = false">
            <div class="flex flex-col justify-center">
              ...
              <form @submit.prevent="handleSubmit" class="mt-2">
                <div class="mb-4">
                  <label for="companyName" class="block text-colorText font-mono mb-2">Company name</label>
                  <input type="text" id="companyName" v-model="companyName" class="shadow border mb-2 border-black rounded-lg w-full py-2 px-3 text-gray-400 leading-tight focus:outline-none focus:shadow-outline bg-colorBackground" required>
                  
                  <label for="companyBio" class="block text-colorText font-mono mb-2">Company biography</label>
                  <textarea id="companyBio" v-model="companyBio" class="shadow  border mb-2 border-black rounded-lg w-full py-2 px-3 text-gray-400 leading-tight focus:outline-none focus:shadow-outline bg-colorBackground" required></textarea>
          ...
          <usureModal :isVisible="showUsure" @close="showUsure = false">
            <div>
              <h2>Are u sure?</h2>
              <div :key="userData.data_id">
                <button class="bg-red-600 p-2 rounded" @click="showUsure = false">no</button>
                <button class="bg-green-600 p-2 rounded" @click="handleDelete(userData.data_id)">{{userData.data_id}}</button>
              </div>
            </div>
          </usureModal>
