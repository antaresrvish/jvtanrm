...
router.put('/updatecomp/:id', upload.fields([
  { name: 'companyAvatar', maxCount: 1 },
  { name: 'companyPhoto', maxCount: 1 },
]), async (req, res) => {
  try {
    const { companyName, companyBio, companyId } = req.body;
    const avatar = req.files['companyAvatar'] ? req.files['companyAvatar'][0].buffer : null;
    const photo = req.files['companyPhoto'] ? req.files['companyPhoto'][0].buffer : null;
    const sql = 'UPDATE user_data SET compname = ?, compbio = ?, compavatar = IFNULL(?, compavatar), comppic = IFNULL(?, comppic) WHERE data_id = ?';
    db.query(sql, [companyName, companyBio, avatar, photo, companyId], (ex, result) => {
      if (ex) {
        console.log("error while inserting to the database", ex);
        return res.status(500).json({ message: 'database error' });
      }
